    ...
    void Update() 
    {
        if (isRobotMoving) 
        {
            ...
            if(startWalkAnimation) 
            {
                ...
                greeterBotAnimator.Play("Walking", 0, 0.0f);
            ...
            Debug.Log((greeterBot.transform.position - greeterBotEndPosition).magnitude);
            if (Vector3.Distance(greeterBot.transform.position, greeterBotEndPosition) < 0.8f) 
            {
                greeterBotAnimator.StopPlayback();

                greeterBotAnimator.Play("Idle_Happy", 0, 0.0f);
                isRobotMoving = false;
            }
        }
    }
